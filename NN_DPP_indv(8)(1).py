...
train_set = datasets.MNIST('data', train = True, download = True, transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))]))
test_set = datasets.MNIST('data', train = False, transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))]))
...
opt = optim.SGD(params = model.parameters(), lr = 0.005, momentum = 0.9, weight_decay = 1e-5)
...
# Constructing optimizer
opt_add_logdet_w1 = optim.SGD(params = model_add_logdet_w1.parameters(), lr = 0.005, momentum = 0.9, weight_decay = 1e-5)
# Constructing scheduler
...
layer1_out = torch.relu(self.w1(x))      
        ...
        '''
'''
((hidden1_size**2) / (784 * hidden2_size))
((784 * hidden2_size) / (hidden1_size**2))
((hidden2_size**2) / (hidden1_size * output_size))
((hidden1_size * output_size) / (hidden2_size**2))
'''
'''
if index == 0:
                L_w1 = F.sigmoid(F.linear(param, param))
                ...
                L_w2_w3 = F.log_softmax(F.linear(param, F.linear(param, L_w2)))
                logdet_w2_w3 = Logdet(L_w2_w3)
                Det_loss += -logdet_w2_w3
                
                L_w1_w2_w3 = F.log_softmax(F.linear(param, F.linear(param, L_w1_w2))) + 0.01 * torch.eye(output_size)
                logdet_w1_w2_w3 = Logdet(L_w1_w2_w3)
                Det_loss += -logdet_w1_w2_w3
                '''
# + 0.1 * torch.eye(hidden1_size) # epsilon term : 0.1
'''
# Using seed
random.seed(400)
torch.manual_seed(400)
    ...
    for batch_idx, (data, target) in enumerate(train_loader_w1_and_w2):
        ...
        for param in model_add_logdet_w1_and_w2.parameters():
            ...
            if index == 0:
                ...
                L_w1 = F.linear(param, param) + 0.01 * torch.eye(hidden1_size) # epsilon term : 0.01
                logdet_w1 = Logdet(L_w1)
                Det_loss += ((param**2).sum() / 784) * torch.trace(L_w1) - logdet_w1
               
            elif index == 1:
                # Add log-det term to loss(w2)
                L_w2 = F.linear(param.transpose(0,1), param.transpose(0,1)) + 0.01 * torch.eye(hidden1_size) # epsilon term : 0.01
                # L_w1_w2 = F.linear(L_w2, L_w1) + 0.01 * torch.eye(hidden1_size) # epsilon term : 0.01
                logdet_w2_t = Logdet(L_w2) 
                Det_loss += ((param.transpose(0,1)**2).sum() / hidden2_size) * torch.trace(L_w2) - logdet_w2_t

                # Add log-det term to loss(w2)
                L_w2 = F.linear(param, param) + 0.01 * torch.eye(hidden2_size) # epsilon term : 0.01
                logdet_w2 = Logdet(L_w2)
                Det_loss += ((param**2).sum() / hidden1_size) * torch.trace(L_w2) - logdet_w2
                ...
                # L_w2_w3 = F.linear(L_w3, L_w2) + 0.01 * torch.eye(hidden2_size) # epsilon term : 0.01
                logdet_w3_t = Logdet(L_w3) 
                Det_loss += ((param.transpose(0,1)**2).sum() / output_size) * torch.trace(L_w3) - logdet_w3_t
...
def train_add_logdet_w1_and_w2_and_w3(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader_w1_and_w2_and_w3):
        ...
        for param in model_add_logdet_w1_and_w2_and_w3.parameters():
            ...
            if index == 0:
                # Add log-det term to loss(w1)
                L_w1 = F.sigmoid(F.linear(param.transpose(0,1), param.transpose(0,1))) + 0.01 * torch.eye(784)
                logdet_w1_t = Logdet(L_w1)
                Det_loss += -((hidden1_size**2) / (784**2)) * logdet_w1_t
                
                # Add log-det term to loss(w1)
                L_w1 = F.sigmoid(F.linear(param, param)) + 0.01 * torch.eye(hidden1_size)
                logdet_w1 = Logdet(L_w1)
                Det_loss += -((784**2) / (hidden1_size**2)) * logdet_w1
                
            elif index == 1:
                # Add log-det term to loss(w2)
                L_w2 = F.sigmoid(F.linear(param.transpose(0,1), param.transpose(0,1))) + 0.01 * torch.eye(hidden1_size)
                logdet_w2_t = Logdet(L_w2)
                Det_loss += -((hidden2_size**2) / (hidden1_size**2)) * logdet_w2_t
               
                # Add log-det term to loss(w2)
                L_w2 = F.sigmoid(F.linear(param, param)) + 0.01 * torch.eye(hidden2_size)
                logdet_w2 = Logdet(L_w2)
                Det_loss += -((hidden1_size**2) / (hidden2_size**2)) * logdet_w2

            elif index == 2:
                # Add log-det term to loss(w2)
                L_w3 = F.log_softmax(F.linear(param.transpose(0,1), param.transpose(0,1))) + 0.01 * torch.eye(hidden2_size)
                logdet_w3_t = Logdet(L_w3)
                Det_loss += -((output_size**2) / (hidden2_size**2)) * logdet_w3_t
               
                # Add log-det term to loss(w2)
                L_w3 = F.log_softmax(F.linear(param, param)) + 0.01 * torch.eye(output_size)
                logdet_w3 = Logdet(L_w3)
                Det_loss += -((hidden2_size**2) / (output_size**2)) * logdet_w3
                
            index += 1

        loss += 10 * Det_loss / len(train_loader.dataset) 
        loss.backward()
        ...
        if batch_idx % 200 == 199:    # print every 200 mini-batches
            print('[%d, %5d] loss: %.6f' %(epoch, batch_idx + 1, running_loss / 200))
            print(logdet_w1_t_av / 200)
            print(logdet_w1_av / 200)
            print(logdet_w2_t_av / 200)
            print(logdet_w2_av / 200)
            print(logdet_w3_t_av / 200)
